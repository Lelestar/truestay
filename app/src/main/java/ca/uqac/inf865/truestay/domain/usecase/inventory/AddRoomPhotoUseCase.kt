...
/**
 * Uploads a photo for a room and adds it to the inventory.
 *
 * @return Result containing the new photo URL on success.
 */
class AddRoomPhotoUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        return try {
            ...
            val path = "inventories/$inventoryId/rooms/$roomId/${System.currentTimeMillis()}"
            ...
            // Find the room and add the photo URL directly to room.photoUrls
            val updatedRooms = inventory.rooms.map { room ->
                if (room.roomId == roomId) {
                    room.copy(photoUrls = room.photoUrls + url)
                } else {
                    room
                }
            }
